...
def import_obstacle(args):
    ...
    # Eliminate rows information of div using sparse matrix operation
    DIV = args.spdiv
    DIV = DIV[np.array(~args.obstacle.flatten(), dtype="bool")]  # Eliminate rows of on stacle

    # Eliminate columns of div
    DIV = DIV[:, np.array(obstacle_mask[np.array(args.mask, dtype="bool")], dtype="bool")]
    args.spdiv = DIV
    args.v_dim = DIV.shape[1]
    print('spdiv has been updated with obstacle')
...
def loadMNIST(args, binary_=True, data_num=1000):
    ...
    # Create a list to store indices for each digit
    indices_per_digit = {i: [] for i in range(10)}

    # Populate the list with indices
    for idx, (image, label) in enumerate(train_data):
        if len(indices_per_digit[label]) < (data_num // 10):
            indices_per_digit[label].append(idx)
        if all(len(indices) == (data_num // 10) for indices in indices_per_digit.values()):
            break

    # Combine the indices from all digits
    subset_indices = [idx for indices in indices_per_digit.values() for idx in indices]
...
def load3dshapes(args, m,n, shapes=None):
  ...
  for shape in shapes:
    ...
    args.channel = shape.shape[2]

    shape = torch.from_numpy(shape)
    X.append( shape  )
    l1_norms.append(shape.sum())
